<template>
  <div class="container">
    <h2 class="homeTitle">
      订单管理<i>{{ days[1] }}</i><span><router-link to="/order">订单详情</router-link></span>
    </h2>
    <div class="orderviewBox">
      <ul>
        <li>
          <span class="status"><i class="iconfont icon-waiting" />待调度</span>
          <span class="num tip"><router-link to="/order?status=2">{{
            orderviewData.waitingOrders
          }}</router-link></span>
        </li>
        <li>
          <span class="status"><i class="iconfont icon-staySway" />制作中</span>
        ...
        <li>
          ...
          <span class="num"><router-link to="/order?status=7">{{
