...
- 定时任务已收口为：
  ...
  - 新增超时未履约日志识别，不改状态
- 当前定时任务仍处于有效状态：
  - 每分钟扫描一次待支付超时订单，超 15 分钟自动取消
  - 每 10 分钟扫描一次制作中 / 待取餐 / 配送中逾期订单，只记日志，不改库状态
- 统计口径已统一为当前 `1-7` 语义。
...
### 第五轮登录链路清理已完成
- `user.openid` 已从当前业务模型中移除。
- `POST /user/user/login` 当前只保留账号密码登录，不再支持微信 `code` 登录。
- `UserLoginVO` 不再返回 `openid`。
- `user` 相关 MyBatis `insert` 映射、演示数据 SQL、初始化 SQL 已同步去掉 `openid`。
- 当前支付链路继续保持模拟支付，`payment(orderNumber)` 和 `paySuccess(orderNumber)` 不依赖 `openid`。
- 仓库已补充增量脚本 `user_openid_drop_update.sql`，用于删除 `user.openid` 列。
...
- 当前 `user.openid` 删除、账号密码登录收口与模拟支付不依赖微信身份的事实也应同步以实际代码为准。
