...
- 当前 FAMILY 评价链路已接入：
  - `status = 6` 的本人订单显示 `评价`
  - 已评价订单显示 `查看评价`
  - 评价弹窗当前只包含星级评分和可选文字评价
  - 本轮不引入图片上传、追评、匿名评价、签收码、签名等复杂能力
...
- 只有 `FAMILY` 可以评价
- 只有 `status = 6` 的已完成订单可以评价
- 当前登录 FAMILY 只能评价自己的订单
- 一个订单只能评价一次
- `score` 只允许 `1~5`
- 未主动评分的已完成订单不会生成评价记录，也不会计入志愿者综合评分
...
- 志愿者概览页当前支持轻量 Excel 导出，导出内容复用当前 overview 数据源，不单独维护报表查询。
- 概览加载失败时，当前前端会显示默认值，不影响任务页继续使用。
- 当前志愿者 `综合评分` 已不再依赖初始化默认值，而是来自真实订单评价聚合结果。
- 若志愿者暂无评价，概览页当前继续显示 `--`。
- 当前志愿者 `等级` 已按累计完成单量落地固定规则：
  - `Level 1：0 ~ 9 单`
  - `Level 2：10 ~ 29 单`
  - `Level 3：30 ~ 59 单`
  - `Level 4：60 ~ 99 单`
  - `Level 5：100 单及以上`
- 当前 `totalOrders / level` 主回写发生在订单完成后；概览查询同时保留兜底计算，避免旧数据导致等级失真。
- 当前 `volunteer_stats.level` 仍保留为结果字段，不做删除；订单完成与评分刷新时都会顺带校正该字段。
- 当前累计服务时长仍优先读取 `volunteer_stats.total_hours`，后续再完善自动回写或重算机制。

### 5.5 OPERATOR 订单页提醒

当前 `OPERATOR` 订单中心已经接入基于 WebSocket 的定向提醒，但提醒对象不再按原始苍穹外卖那种方式广播给管理端，而是按助餐点定向推送给在线操作员。

当前规则包括：

- WebSocket 连接当前使用：`/ws/{empId}/{role}/{diningPointId}`
- 服务端当前按以下条件过滤提醒接收者：
  - `role = OPERATOR`
  - `diningPointId = orders.dining_point_id`
- FAMILY 支付成功后，会向订单所属助餐点的在线 `OPERATOR` 推送“来单提醒”
- FAMILY 催单后，会向订单所属助餐点的在线 `OPERATOR` 推送“催单提醒”
- 当前 WebSocket 消息体统一包含：
  - `type`
  - `orderId`
  - `orderNumber`
  - `diningPointId`
...
- 以下状态当前不允许催单：
  ...
  - `6 已完成`
  - `7 已取消`
- 同一订单当前增加了 60 秒基础催单限流，避免短时间重复刷屏
- 前端当前只在 `OPERATOR` 的 `/order` 页面建立并消费 WebSocket 连接，不再在全局 Navbar 中消费订单提醒
- 当前只有 `OPERATOR` 订单页会播放提醒音：
  - `preview.mp3`：来单提醒
  - `reminder.mp3`：催单提醒

说明：

- `ADMIN` 本轮仍可继续看全局订单，但不再作为本轮来单/催单提醒的主要消费对象。
- 当前 WebSocket 基础设施仍依赖 URL 参数携带身份信息，不是完整的服务端鉴权绑定连接；本轮只做最小改动收口。

### 5.6 前端视觉与站点标识

当前前端已经完成一轮“去苍穹外卖化”的视觉收口，但不涉及任何业务逻辑变更。

当前已落地的视觉调整包括：

- 全站主色已由原先偏黄黑的外卖后台风格，统一收口为社区青绿系
- Element UI 通用主色、按钮、页签、分页、选中态、侧栏高亮等全局样式已统一按新主题覆盖
- 登录页、点餐页、历史订单页、订单中心、志愿者页面等主要运行页面的旧黄品牌色已按当前主题清洗
- 登录页当前已去掉原左侧大图，改为纯样式的双栏信息版，不再依赖 `src/assets/login/login-l.png`
- 登录页当前增加了轻量首屏动画，包括卡片入场、信息分段淡入和背景装饰缓慢漂浮；仅作用于视觉层，不改变登录行为
- 侧栏顶部 logo 区块已整体移除，不再保留 `sidebar-logo` 和折叠态 mini logo
- 浏览器标签页与 PWA 相关图标已整套替换为新的站点图标：
  - 风格固定为“圆角方块 + 白色餐碗”
  - 主色使用当前项目青绿 `#2F8F83`
  - 包括 `favicon.ico`、`public/img/icons` 下各尺寸 PNG、`apple-touch-icon`、`safari-pinned-tab.svg`
- `public/index.html` 当前已恢复显式图标声明，`public/manifest.json` 也已指向新图标资源

说明：

- 本轮视觉改造只处理样式和静态图标资源，不修改任何接口、权限、下单、支付、WebSocket、评价、志愿者任务等业务逻辑。
- 当前浏览器若仍短暂显示旧标签图标，通常属于本地缓存；强制刷新或清除站点缓存后应以当前仓库中的新图标为准。
...
- 本轮评分系统必须执行 `order_review_rating_update.sql`
  - 否则 `order_review` 表不存在时，FAMILY 历史订单页会在查询 `reviewed` 状态时报 `500`
...
- 定时任务当前只保留合理逻辑：
  ...
  - 每 10 分钟执行一次超时履约识别，只做日志/统计，不改订单状态
- 异常履约识别口径：
  ...
  - `4 待取餐` 超时：待取餐超时
  - `5 配送中` 超时：配送中超时
- 当前代码中的订单定时处理机制仍然有效，但仅限上述两类安全任务：
  - 自动取消超时未支付订单
  - 记录异常履约告警日志
- 当前代码里不存在“自动把配送中订单推进到已完成”的定时处理逻辑。
- 统计口径统一按当前状态语义执行：
  - `1` 待支付
  - `2` 待调度
  - `3` 制作中
...
### 10.13 2026-03 管理员端查询增强

本轮对管理员端多个列表页补齐了轻量筛选能力，目标是增强日常管理效率，不改页面主体结构，不新增独立筛选面板。
...
- 员工管理页 `/employee`
  - 当前支持按：
    ...
    进行组合筛选
  - 对应分页接口当前支持：
    - `GET /admin/employee/page?page=&pageSize=&name=&role=&diningPointId=`
- 助餐点管理页 `/diningPoint`
  - 当前支持按：
    - 助餐点名称
    - 状态：`全部 / 营业中 / 休息中`
    进行组合筛选
  - 对应分页接口当前支持：
    - `GET /admin/diningPoint/page?page=&pageSize=&name=&status=`
- 老人档案页 `/elderly`
  - 当前支持按：
    - 老人姓名
    - 所属助餐点：`全部 / 具体助餐点`
    进行组合筛选
  - 对应分页接口当前支持：
    - `GET /admin/elderly/page?page=&pageSize=&name=&diningPointId=`

说明：

- 这轮只补齐查询条件，不改新增/编辑表单结构。
- 老人档案页的“查询用助餐点下拉”和“表单用助餐点下拉”当前口径不同：
  - 查询下拉允许加载全部助餐点，便于回查历史数据
  - 表单下拉仍只加载启用中的助餐点，保持原有保存规则
